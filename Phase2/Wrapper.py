...
def render(model, model_fine, rays_origin, rays_direction, tn=2, tf=6, samples=192, clear_bg=True):
    ...
    norm_weights = T*alpha/(torch.sum(T*alpha, dim=-1, keepdim=True) + 1e-5)
    ...
    #combine the coarse and fine samples
    ...
    #get the color and opacity values for the points
    ...
    if clear_bg:
        ...
        C_r_fine = (weights_fine*C_hat_fine).sum(1)#weights_fine.unsqueeze(2)
...
def Loss(groundtruth, prediction, prediction_fine):
    ...
    norm_loss = (((prediction - groundtruth).norm())**2 + ((prediction_fine - groundtruth).norm())**2).mean()
...
def test(args, mode = "test", epoch = 0):
    ...
    cv2.imwrite(os.path.join(imgs_path + "_test_.png"),rgb2bgr)
